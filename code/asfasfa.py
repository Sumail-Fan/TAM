from model import createGraph,findHighestSimilarityRankTopic,findHighestSimilarityRank
import torch
import os
...
sajfi = cosine(reduced_embedding[0],reduced_embedding[1])
sajfias = cosine(sentence_embedding[0],sentence_embedding[1])
...
for i in range(len(article)):
    # min_dis_list.append(cosine(np.array(reduced_embedding[i]),np.array(centers[index_list[-1]])))
    dis = []
    for center in centers_less:
        dis.append(cosine(np.array(reduced_embedding[i]), np.array(center)))
    weights.append(max(dis))

newRank = findHighestSimilarityRankTopic(sentenceGraph,[1.0]*len(article),weights)
ans = np.argpartition(newRank, -3)[-3:]

sentenceGraphRe = createGraph(reduced_embedding)
newRankRe = findHighestSimilarityRank(sentenceGraphRe,[1.0]*len(article),False)
newRankOri = findHighestSimilarityRank(sentenceGraph,[1.0]*len(article),False)

ansor = np.argpartition(newRankOri, -3)[-3:]
ans12 = np.argpartition(newRankRe, -3)[-3:]

score = get_score(article,abstract,ans)
scoreori = get_score(article,abstract,ansor)
score12 = get_score(article,abstract,ans12)
